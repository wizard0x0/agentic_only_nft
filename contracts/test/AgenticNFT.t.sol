...
contract AgenticNFTTest {
    ...
    function setUp() external {
        nft = new AgenticNFT("AgenticNFT", "ANFT", BASE_URI);
    ...
    function test_name() external view {
        require(
            keccak256(bytes(nft.name())) == keccak256(bytes("AgenticNFT")),
            "wrong name"
        );
    }
    ...
    function test_symbol() external view {
        require(
            keccak256(bytes(nft.symbol())) == keccak256(bytes("ANFT")),
            "wrong symbol"
        );
    }
    ...
    function test_baseURI() external view {
        require(
            keccak256(bytes(nft.baseURI())) == keccak256(bytes(BASE_URI)),
            "wrong base URI"
        );
    }
    ...
    function test_canMint_EOA_returnsFalse() external view {
        // Use a known EOA-like address (no code)
    ...
    // Note: We cannot simulate a pure EOA call from within Solidity (all callers in
    // Solidity tests are contracts). The canMint() unit tests above verify the logic.
    // Integration/end-to-end verification is covered in the JS tests (test/) or
    // by calling from a wallet CLI directly.
    ...
    function test_owner_canUpdateBaseURI() external {
        ...
        require(
            keccak256(bytes(nft.baseURI())) == keccak256(bytes(newURI)),
